...
class DeepLearningModel:
    ...
    def __init__(self, num_rnn_layer, units, use_attention, initializer, activation, learning_rate, padding, epochs, batch_size, num_fold_training,
                 val_threshold, make_balance_dataset, make_balance_batch, weighted_logits, val_metric):
    ...
    def classifier(self, input_layer, num_class):
        for i in range(self.num_rnn_layer):
            with tf.variable_scope('rnn_set_'+str(i), initializer=self.initializer):
                lstm = tf.contrib.rnn.BasicLSTMCell(self.units)
                rnn_layer, _ = tf.nn.dynamic_rnn(lstm, input_layer, dtype=tf.float32)
                input_layer = rnn_layer


        # Apply attention model
        if self.use_attention:
            rnn_layer = self.attention(rnn_layer)
        else:
            rnn_layer = rnn_layer[:, -1, :]

        dense = tf.layers.dropout(rnn_layer, 0.5, name='drop_out')
        classifier_output = tf.layers.dense(dense, num_class, name='softmax_layer')
        return classifier_output

    def attention(self, rnn_layer):
        # conv_layer = [batch_size, num_convolved_legs, leg_vector_size]

        leg_vector_size = rnn_layer.get_shape().as_list()[-1]  # get last dimension
        with tf.variable_scope('attention_set_', initializer=self.initializer):
            # v_attention: context vector
            v_attention = tf.get_variable("attention_1", shape=[leg_vector_size])

            # 1.one-layer MLP
            u = tf.layers.dense(rnn_layer, leg_vector_size, activation=tf.nn.tanh,
            ...
            # 3.weight sum
            attention_representation = tf.reduce_sum(tf.multiply(rnn_layer, normalized_weight), axis=1)  # [batch_size,num_units]
    ...
    def training(self, one_fold):
        Train_X = one_fold[0]
        Train_X = np.squeeze(Train_X)  # remove the last dimension, which was added for CNN
        Train_Y = one_fold[1]
        Val_X = one_fold[2]
        Val_X = np.squeeze(Val_X)  # remove the last dimension, which was added for CNN

        Val_Y = one_fold[3]
        Test_X = one_fold[4]
        Test_X = np.squeeze(Test_X)  # remove the last dimension, which was added for CNN
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                print('====================================================')
                saver.save(sess, r"C:/Users/sinad/Desktop/CATT_Intern/tenserflow_results/Rnn-noAtt", global_step=k)
            ...
            saver.restore(sess, r"C:/Users/sinad/Desktop/CATT_Intern/tenserflow_results/Rnn-noAtt-"+str(max_metric[0]))
...
if __name__ == '__main__':
    deep_learning_model = DeepLearningModel(num_rnn_layer=1, units=100, use_attention=0, initializer=tf.contrib.layers.xavier_initializer(uniform=True, seed=None, dtype=tf.float32),
                                            activation=tf.nn.relu, learning_rate=0.001, padding='valid', epochs=20,
                                            batch_size=256,
                                            num_fold_training='All', val_threshold=0.0001,
                                            make_balance_dataset=0, make_balance_batch=1,
                                            weighted_logits=[1., 1., 1.], val_metric='recall')
