...
class DeepLearningModel:
    ...
    def __init__(self, num_filter, num_dense, num_conv_set, use_attention, initializer, kernel_size, strides, activation, padding, learning_rate, epochs, batch_size, num_fold_training, val_threshold,
    ...
    def classifier(self, input_layer, num_class):
        ...
        for i in range(self.num_conv_set):
            with tf.variable_scope('conv_set_'+str(i), initializer=self.initializer):
                kernel_size_ = [self.kernel_size[i]] + [input_layer.get_shape().as_list()[2]]
                conv_layer = tf.layers.conv2d(inputs=input_layer, activation=self.activation, filters=self.num_filter[i],
                                              kernel_size=kernel_size_, strides=self.strides, padding=self.padding)

                num_sequence = conv_layer.get_shape().as_list()[1]
                pool_size = [num_sequence, 1]
                pool_layer = tf.layers.max_pooling2d(conv_layer, pool_size=pool_size, strides=pool_size, name='pool')
                pool_layer = tf.reshape(pool_layer, shape=[-1, self.num_filter[i]])
                pool_output.append(pool_layer)

        # Concatenate all pooled layers and create a 1-d vector
        dense = tf.concat(pool_output, axis=1, name='concatenation')
        units = int(dense.get_shape().as_list()[-1] / 2)
        with tf.variable_scope('dense_set_1_', reuse=tf.AUTO_REUSE, initializer=self.initializer):
    ...
    def attention(self, conv_layer):
        ...
        with tf.variable_scope('attention_set_', reuse=tf.AUTO_REUSE, initializer=self.initializer):
            # v_attention: context vector
            v_attention = tf.get_variable("attention_1", shape=[leg_vector_size])

            # 1.one-layer MLP
            u = tf.layers.dense(conv_layer, leg_vector_size, activation=tf.nn.tanh,
                                use_bias=True)  # [batch_size, num_convolved_legs, leg_vector_size].no-linear
    ...
    def prediction_prob(self, Test_X, classifier_output, input_layer, sess):
        prediction = []
        for i in range(len(Test_X) // self.batch_size):
            Test_X_batch = Test_X[i * self.batch_size:(i + 1) * self.batch_size]
            prediction.append(sess.run(tf.nn.softmax(classifier_output), feed_dict={input_layer: Test_X_batch}))
        Test_X_batch = Test_X[(i + 1) * self.batch_size:]
        if len(Test_X_batch) >= 1:
            prediction.append(sess.run(tf.nn.softmax(classifier_output), feed_dict={input_layer: Test_X_batch}))
        prediction = np.vstack(tuple(prediction))
        y_pred = np.argmax(prediction, axis=1)
    ...
    def training(self, one_fold):
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                print('====================================================')
                saver.save(sess, r"C:/Users/sinad/Desktop/CATT_Intern/tenserflow_results/CNN-model", global_step=k)
            ...
            saver.restore(sess, r"C:/Users/sinad/Desktop/CATT_Intern/tenserflow_results/CNN-model-"+str(max_metric[0]))
...
if __name__ == '__main__':
    ...
    deep_learning_model = DeepLearningModel(num_filter=[100, 100], num_dense=0, num_conv_set=2, use_attention=0, initializer=tf.contrib.layers.xavier_initializer(uniform=True, seed=7),
                                        kernel_size=[2, 2], strides=1, activation=tf.nn.relu, learning_rate=0.001, padding='valid', epochs=20, batch_size=16, num_fold_training='All', val_threshold=0.0001,
                                            make_balance_dataset=0, make_balance_batch=1, alpha=1., weighted_logits=[1, 1., 1.], val_metric='recall')

    filename = 'kfold_dataset_DL_light&mid&heavy_mean_std_OSRM_.pickle'
